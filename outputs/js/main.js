...
function initIconsWithRetry(attempts) {
    if (typeof lucide !== 'undefined') {
        lucide.createIcons();
    } else if (attempts > 0) {
        setTimeout(() => initIconsWithRetry(attempts - 1), 300);
    }
}

// Initialize on DOM Load
document.addEventListener('DOMContentLoaded', () => {
    // Current Year footer anchor (desktop + mobile)
    const yearEl = document.getElementById('current-year');
    if (yearEl) yearEl.textContent = new Date().getFullYear();
    const mobileYearEl = document.querySelector('.mobile-year');
    if (mobileYearEl) mobileYearEl.textContent = new Date().getFullYear();

    // Initialize Lucide Icons with retry for CDN / Script load delays
    initIconsWithRetry(10);
...
function toggleMobileMenu() {
    ...
    if (menu.classList.contains('hidden')) {
        ...
        if (icon.tagName.toLowerCase() === 'svg') {
            icon.innerHTML = '<line x1="4" y1="6" x2="20" y2="6"></line><line x1="4" y1="12" x2="20" y2="12"></line><line x1="4" y1="18" x2="20" y2="18"></line>';
        }
    }
}
    ...
    requestAnimationFrame(() => {
        ...
        if (dashboard) {
            ...
            if (dbRect.top < window.innerHeight) {
                ...
                // Mobile: create a dedicated IntersectionObserver so the dashboard
                // animations (channel bars, revenue line, metric rings) still fire
                // when scrolled into view, even though the reveal observer is disabled.
